...
function createChildEnv(): NodeJS.ProcessEnv {
  ...
  if (
    ...
    return {
      ...withoutNodePath,
      // Package installers commonly use this standard signal to skip prompts.
      // A detached child also has no controlling terminal on POSIX.
      CI: '1',
    }
  }
...
async function executeShellCommand(
  ...
  const child = spawn(input.command, {
    shell: true,
    env: childEnv,
    // A new POSIX session prevents commands that open /dev/tty from blocking
    // the desktop process terminal after stdio has been intentionally ignored.
    // Keep the established Windows child-process behaviour.
